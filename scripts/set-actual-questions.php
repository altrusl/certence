...
foreach ($ids as $oid => $id) {
    if (file_exists(__DIR__ . "/discussions/actual/$id.json")) {
        echo "\n111----------------------------------".$oid;
        if (!key_exists($oid, $data['amazon']['aws-certified-solutions-architect-associate-saa-c02']['tags'])) {
...
file_put_contents(__DIR__ . "/users/data.json", json_encode($data, JSON_PRETTY_PRINT));
